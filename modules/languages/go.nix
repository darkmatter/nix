...
in
{
  ...
  config = lib.mkIf config.darkmatter.go.enable {
    ...
    env = lib.mkIf config.darkmatter.go.enableCGO {
      ...
      MACOSX_DEPLOYMENT_TARGET = if pkgs.stdenv.isDarwin then "14.0" else "";
      CGO_CFLAGS = if pkgs.stdenv.isDarwin then "-mmacosx-version-min=14.0" else "";
      CGO_LDFLAGS = if pkgs.stdenv.isDarwin then "-mmacosx-version-min=14.0" else "";
    };
